...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    // The exit fade is NOT on a timer — it starts inside _doNavigate(), so the
    // splash can never fade out while we are still waiting for the connectivity
    // check. (Both used to fire at 3.2s, which hid the coupling; at 1.2s the
    // check has only ~400ms of slack and a slow network would fade the splash
    // to nothing with no page behind it.)
  ...
  Future<void> _doNavigate() async {
    ...
    _navigated = true;
    _navTimer?.cancel();
    // Fade out only now that we know we are actually leaving. The new page's
    // 320ms slide-in overlaps this 700ms fade, so there is no black frame.
    _exitCtrl.forward();
