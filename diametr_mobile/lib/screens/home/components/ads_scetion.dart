...
class _AdsSectionState extends State<AdsSection> {
  ...
  /// Open the entity a banner links to (shop / product / worker / region).
  /// A banner with no target is static and simply does nothing on tap.
  void _openAd(BuildContext context, dynamic item, String name) {
    final type = item['type']?.toString();
    if (type == 'PRODUCT' && item['product_id'] != null) {
      Navigator.of(context).pushNamed('/productScreen',
          arguments: {"product_id": item['product_id'], "name": name});
    } else if (type == 'SHOP' && item['shop_id'] != null) {
      Navigator.of(context).pushNamed('/marketScreen',
          arguments: {"id": item['shop_id'].toString(), "name": name});
    } else if (type == 'WORKER' && item['worker_id'] != null) {
      Navigator.of(context).pushNamed('/workerScreen',
          arguments: {"id": item['worker_id'].toString(), "name": name});
    } else if (type == 'REGION') {
      Navigator.of(context).pushNamed('/marketAllScreen');
    }
  }
          ...
          child: Column(
            ...
            CarouselSlider.builder(
              ...
              itemBuilder: (context, index, _) {
                ...
                // Bilingual banner text: show the Russian variant for ru locale,
                // falling back to the Uzbek/default title & subtitle.
                final lang = context.locale.languageCode;
                final title = ((lang == 'ru' ? item['title_ru'] : item['title']) ??
                        item['title'] ??
                        '')
                    .toString();
                final subtitle =
                    ((lang == 'ru' ? item['subtitle_ru'] : item['subtitle']) ??
                            item['subtitle'] ??
                            '')
                        .toString();

                return GestureDetector(
                  onTap: () => _openAd(context, item, title),
                  child: Container(
