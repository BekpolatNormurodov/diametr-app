                        ...
                        SizedBox(
                          ...
                          child: ListView(
                            ...
                            children: [
                              ...
                              ...categories.map((cat) {
                                final id = cat["id"];
                                final name = ((context.locale.languageCode ==
                                            'ru'
                                        ? cat["name_ru"]
                                        : cat["name_uz"]) ??
                                    cat["name"] ??
...
class _ProductCardState extends State<_ProductCard> {
  ...
  /// Lowest working shop price across the product's variants (or null).
  static num? _minVariantPrice(Map item) {
    num? min;
    final items = item["items"];
    if (items is List) {
      for (final it in items) {
        final sps = (it is Map) ? it["shop_products"] : null;
        if (sps is List) {
          for (final sp in sps) {
            if (sp is! Map) continue;
            final bonus = sp["bonus_price"];
            final regular = sp["price"];
            final eff = (bonus is num && bonus > 0 &&
                    (regular is! num || bonus < regular))
                ? bonus
                : regular;
            if (eff is num && (min == null || eff < min)) min = eff;
          }
        }
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final String name = (lang == 'ru'
                ? widget.item["name_ru"] ?? widget.item["name_uz"]
                : widget.item["name_uz"] ?? widget.item["name_ru"])
            ?.toString() ??
