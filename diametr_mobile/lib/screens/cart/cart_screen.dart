    ...
    for (final entry in _grouped.entries) {
      ...
      try {
        ...
        final resp = await client.post(
          ...
          data: {
            "shop_id": int.tryParse(shopId.toString()) ?? shopId,
            // Backend DELIVERY_TYPE enum is UPPERCASE (MARKET/YANDEX/FIXED) —
            // sending it lower-cased made every order fail validation.
            "delivery_type": type,
            "amount": amount,
            if (location != null) "lat": location["lat"],
            if (location != null) "lon": location["lon"],
            "source": "MOBILE",
            // Order products must be {shop_product_id, count}; the cart item's
            // `id` IS the shop_product id. whitelist strips any other field.
            "products": shopItems
                .map((it) => {
                      "shop_product_id":
                          int.tryParse(it["id"].toString()) ?? it["id"],
                      "count": it["count"],
                    })
                .toList(),
          },
