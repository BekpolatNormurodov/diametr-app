...
class Endpoints {
  // Tuned for slow / congested mobile networks (2G/EDGE, weak WiFi). The API is
  // gzipped so payloads are small, but a weak link still needs headroom to
  // finish a response; a receiveTimeout that is too tight turns a slow-but-alive
  // network into a hard failure. connectTimeout stays tighter so a truly dead
  // network surfaces the error quickly instead of hanging.
