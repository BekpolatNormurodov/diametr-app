...
export default function PaymentsTable({ data, onRefetch }: { data: PaymentItemProps[]; onRefetch?: () => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // `type` is the SHOP|WORKER|AD enum (what the payment is FOR), NOT a
      // payment method — sending 'CARD'/'CASH' here 400'd every save. It is not
      // editable from this modal, so omit it entirely.
      const payload: any = {};
