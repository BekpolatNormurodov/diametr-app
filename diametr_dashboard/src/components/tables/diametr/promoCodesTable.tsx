...
export default function PromoCodesTable({
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // On edit, send explicit null (not undefined) when an optional field is
      // cleared so the backend actually nulls the column; on create, keep
      // undefined so omitted fields fall back to their defaults.
      const blank = editItem ? null : undefined;
      const payload = {
        code: form.code,
        discount_type: form.discount_type,
        discount_value: Number(form.discount_value),
        min_order_amount: form.min_order_amount ? Number(form.min_order_amount) : blank,
        max_uses: form.max_uses ? Number(form.max_uses) : blank,
        expires_at: form.expires_at || blank,
      };
      if (editItem) {
