...
export default function ServicesTable({ data, onRefetch }: { data: ServiceItemProps[]; onRefetch?: () => void }) {
  ...
  const filteredData = search.trim() === "" ? tableData : tableData.filter((s) => { const q = search.toLowerCase(); return (s.name ?? "").toLowerCase().includes(q) || (s.desc ?? "").toLowerCase().includes(q); });
  ...
  const openEdit = (item: ServiceItemProps) => {
    setEditItem(item);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        await axiosClient.put(`/service/${editItem.id}`, { name: form.name, desc: form.desc });
  ...
  const handleExport = () => {
    const ws = XLSX.utils.json_to_sheet(tableData.map((s) => ({
      ID: s.id, Nomi: s.name ?? "", Tavsif: s.desc ?? "", Yaratilgan: Moment(s.createdt ?? s.createdAt).format("DD.MM.YYYY"),
    })));
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Services");
    XLSX.writeFile(wb, `services-${Moment().format("YYYY-MM-DD")}.xlsx`);
  };
            ...
            {currentItems.length === 0 ? (
              ...
              <TableRow key={item.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                ...
                <TableCell className="px-5 py-4">
                  {item.image ? (
                    <img src={`${staticUrl}/static/services/${item.image}`} alt={item.name} className="w-10 h-10 rounded-xl object-cover ring-2 ring-white dark:ring-white/[0.06] shadow-sm" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                ...
                <TableCell className="px-5 py-4 text-sm text-gray-500 dark:text-gray-400 max-w-[250px] truncate">{item.desc ?? "-"}</TableCell>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-[500px] m-4">
        <div className="relative w-full p-4 overflow-y-auto bg-white no-scrollbar rounded-3xl dark:bg-gray-900 lg:p-8">
          <div className="px-2 pr-14 mb-6">
            <h4 className="text-xl font-semibold text-gray-800 dark:text-white">Xizmatni tahrirlash</h4>
          </div>
          <div className="flex flex-col gap-4 px-2">
            <div>
              <Label>Nomi</Label>
              <Input type="text" placeholder="Xizmat nomi" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
            </div>
            <div>
              <Label>Tavsif</Label>
              <Input type="text" placeholder="Qisqa tavsif" value={form.desc} onChange={(e) => setForm({ ...form, desc: e.target.value })} />
            </div>
          </div>
