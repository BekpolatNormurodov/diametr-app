...
export default function RegionsTable({ data, onRefetch }: { data: RegionItemProps[]; onRefetch?: () => void }) {
  ...
  const filteredData = searchQueryKey === "" ? tableData : tableData.filter((s) => matchesSearchKey(searchQueryKey, [s.name]));
  ...
  const handleExport = () => {
    const ws = XLSX.utils.json_to_sheet(tableData.map((r) => ({ ID: r.id, Nomi: r.name ?? "", Yaratilgan: Moment(r.createdt ?? r.createdAt).format("DD.MM.YYYY") })));
